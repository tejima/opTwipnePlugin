...
class TestTask extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->namespace        = 'pne';
    $this->name             = '20090808test';
    $this->aliases          = array('pne-test');
...
EOF;
    ...
    $this->addOption('qname', null, sfCommandOption::PARAMETER_OPTIONAL, 'queue name', null);
  }
...
$m =  Doctrine::getTable('Member')->find(1);
  ...
  private function test_pne_exclude(){
    $keywords = array('RT','@','||');
  ...
  private function test_pne_community(){
    echo realpath('./') . "\n";
...
EOF;
  ...
  private function test_sepalator(){
  ...
  private function test_member_config_checkbox(){
    $member = Doctrine::getTable('Member')->find(2);
    print_r($member->getConfig('SEPALATOR'));
  }
  private function test_member_config(){

    $member = Doctrine::getTable('Member')->find(2);
    print_r($member->getConfig('TWITTERDIRECT'));

  }
  private function test_readonly_calendar(){
    $user = 'USER';
    $pass = 'PASSWORD';
...
*/
  ...
  private function test_recurrence(){
    $user = 'GOOGLEUSER';
    $pass = 'GOOGLEPASSWORD';
    $service = Zend_Gdata_Calendar::AUTH_SERVICE_NAME;
    $client = Zend_Gdata_ClientLogin::getHttpClient($user,$pass,$service);
  ...
  private function test_calendar($options){
    $this->processCalendar('GOOGLEUSER','GOOGLEPASSWORD');
  }
  private function processCalendar($user,$pass){
    $service = Zend_Gdata_Calendar::AUTH_SERVICE_NAME;
    $client = Zend_Gdata_ClientLogin::getHttpClient($user,$pass,$service);
    $service = new Zend_Gdata_Calendar($client);
    try{
  ...
  private function test_parse_str($options){
  ...
  private function test_exec(){
    echo realpath('./') . "\n";
    $command = 'ruby '. realpath('./') . "/plugins/twipnePlugin/voice.rb";
    echo $command . "\n";
    echo exec($command);
  }
  private function test_browser(){
...
//http://sns.openpne.jp/?m=pc&a=page_o_help_login_error
  ...
  private function test_array_intersect(){
    $array1 = array('つ','い','た');
    $array2 = array('い','た','つ');
  ...
  private function test_timeline(){
    //date_default_timezone_set('Asia/Tokyo');
  ...
  private function test_option($options){
     $result = $options['mode'];
     print_r($result);
  }
  private function test_auto(){
  ...
  private function test_get_twitterfoxtext(){
  ...
  private function test_get_member4config(){
    $q = Doctrine_Query::create()
  ...
  private function test_reverse_search($name = 'tejicube'){
   $q = Doctrine_Query::create()
    ->select('mc.member_id')
  ...
  private function test_insert_config(){
    $config = new SnsConfig();
    $config->name = 'test_japanese';
    $config->value = 'ぼぢー';
    $config->save();
  }
  private function test_insert_entry(){
    $data = new Entry();
    $data->body = '日本語ぼでい';
    $data->meta = 'にほんごめた';
    $data->member_id = 2;
